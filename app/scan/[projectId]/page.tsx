...
export default function ScanProgressPage() {
  ...
  // Debug log
  useEffect(() => {
    setDebugInfo(`Project ID: ${projectId || 'none'}`)
  }, [projectId])
    ...
    async function fetchProject() {
      try {
        ...
        if (res.ok) {
          ...
          setDebugInfo(`Loaded project: ${data.project.name}`)
        }
      } catch (err) {
        console.error('Failed to fetch project:', err)
        setDebugInfo(`Error fetching project: ${err}`)
      }
    }
    fetchProject()
  }, [projectId])
  ...
  useEffect(() => {
    ...
    async function runScan() {
      try {
        ...
        setDebugInfo('Scan started, running phases...')
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-900 to-slate-800 text-white">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-12">
        <div className="text-xs text-slate-500 mb-4">Debug: {debugInfo}</div>
